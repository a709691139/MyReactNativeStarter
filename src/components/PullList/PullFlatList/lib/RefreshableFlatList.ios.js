...
export default class RefreshableFlatList extends PureComponent {
  static propTypes = {
    ...
    topIndicatorComponent: PropTypes.oneOfType([
      PropTypes.func,
      PropTypes.element
    ]),
    bottomIndicatorComponent: PropTypes.oneOfType([
      PropTypes.func,
      PropTypes.element
    ]),
    topPullingIndicator: PropTypes.oneOfType([
      PropTypes.string,
      PropTypes.element
    ]),
    topHoldingIndicator: PropTypes.oneOfType([
      PropTypes.string,
      PropTypes.element
    ]),
    topRefreshingIndicator: PropTypes.oneOfType([
      PropTypes.string,
      PropTypes.element
    ]),
    bottomPullingIndicator: PropTypes.oneOfType([
      PropTypes.string,
      PropTypes.element
    ]),
    bottomHoldingIndicator: PropTypes.oneOfType([
      PropTypes.string,
      PropTypes.element
    ]),
    bottomRefreshingIndicator: PropTypes.oneOfType([
      PropTypes.string,
      PropTypes.element
    ]),
    topPullingPrompt: PropTypes.oneOfType([
      PropTypes.string,
      PropTypes.element
    ]),
    topHoldingPrompt: PropTypes.oneOfType([
      PropTypes.string,
      PropTypes.element
    ]),
  ...
  static defaultProps = {
    ...
    topPullingPrompt: "下拉即可刷新...",
    topHoldingPrompt: "释放即可刷新...",
    topRefreshingPrompt: "加载中...",
    bottomPullingPrompt: "上拉即可刷新...",
    bottomHoldingPrompt: "释放即可刷新...",
  ...
  onScroll = event => {
    const offsetY =
      event.nativeEvent.contentOffset.y + event.nativeEvent.contentInset.top;
    ...
    if (!this.state.releasing) {
      if (offsetY < -this.props.minPullDownDistance) {
        ...
        (contentOffset < 0 &&
          offsetY >= this.props.minPullUpDistance &&
          offsetY > 0)
      ) {
        this.setState({
          offsetY,
  ...
  renderIndicator(indicator) {
    if (
      !this.props.showTopIndicator &&
      this.state.scrollStatus.indexOf("DOWN") > -1
    ) {
      return null;
    } else if (
      !this.props.showBottomIndicator &&
      this.state.scrollStatus.indexOf("UP") > -1
    ) {
      return null;
    }
    ...
    if (isValidElement(indicator)) {
      delete refreshingIndicatorProps.styles;
